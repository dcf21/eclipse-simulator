...
image_ptr image_get(const char *filename) {
    ...
    if (setjmp(png_jmpbuf(png_ptr))) {
        logging_error("Unexpected error in libpng while trying to decode PNG image file");
        goto finalise;
    }
    ...
    if (png_colour_type & PNG_COLOR_MASK_ALPHA) {
        if (DEBUG) logging_log("PNG uses transparency");
        if (png_get_bKGD(png_ptr, info_ptr, &backgndp)) {
            if (DEBUG) logging_log("PNG defines a background colour");
            png_set_background(png_ptr, backgndp, PNG_BACKGROUND_GAMMA_FILE, 1, 1.0);
        } else {
            if (DEBUG) logging_log("PNG does not define a background colour");
    ...
    if (png_colour_type == PNG_COLOR_TYPE_PALETTE) {
        ...
        if (i == 0) {
            logging_error("PNG image file claims to be paletted, but no palette was found");
            goto finalise;
        }
        pal_len = ncols;
    ...
    if (png_get_valid(png_ptr, info_ptr, PNG_INFO_tRNS)) {
        if (DEBUG) logging_log("PNG has transparency");
