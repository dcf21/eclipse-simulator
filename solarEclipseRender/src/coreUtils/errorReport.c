...
void logging_fatal(char *file, int line, char *msg) {
    ...
    logging_error(introline);
    logging_error(temp_stringE);
    if (DEBUG) logging_log("Terminating with error condition 1.");
    exit(1);
}
...
//! logging_log - Log an information message
